...
def get_weather_info(
    ...
    if isinstance(weather_info, list): # 天気コードに基づいて説明とアイコンファイル名を取得
        return {"description": weather_info[3], "icon": weather_info[0]} # [3]は天気の説明, [0]はアイコンのファイル名
    else:
        return weather_info
                ...
                if "areas" in series:  # areasがある場合
                    for area in series["areas"]:  # areasのデータを取得
                        ...
                        if "weatherCodes" in area:  # weatherCodesがある場合
                            for i, code in enumerate(area["weatherCodes"]): #enumerate関数でインデックス番号と要素を取得
